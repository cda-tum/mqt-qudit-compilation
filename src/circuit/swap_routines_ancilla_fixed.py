...
def rotate_over_ancilla(path, source, target, placement, dimension):
    ...
    target = (
        target + 1
    )  # because target is signaling the first encountered negative node
    ...
    sensitivity_source = [
        placement.get_edge_sensitivity(path[source], node) for node in neighbours_source
    ]
    sensitivity_target = [
        placement.get_edge_sensitivity(path[target], node) for node in neighbours_target
    ]
    ...
    while i > 0:
        ...
        if path[i] >= 0:
            ...
            if next_swap == 0 and path[next_swap] == source:
                ...
                routine_cost, sequence, placement, invert_routing = rotate_over_ancilla(
                    path, next_swap, i, placement, dimension
                )
                ...
                if next_swap > 0:
                    routine_cost, sequence, placement, path = pi_swap_routine(
                        path, i, next_swap, placement, dimension
                    )
                    pi_pulses_routing += sequence
                    cost_of_pi_pulses += routine_cost
