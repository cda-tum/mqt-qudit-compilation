...
class QuantumCircuit:
    ...
    def PI_PULSE(self, qudit_line, lev_a, lev_b, additional_bookmark, seq_flag):
        ...
        self.qreg[qudit_line].append(
            PI_PULSE(lev_a, lev_b, additional_bookmark, seq_flag, self.dimension)
        )
                ...
                if key2 in levels[key1]:
                    ...
                    for tag in levels[key1]:
                        if tag < key2:
                            ...
                            levels[key1][tag] = np.mod(
                                (levels[key1][tag] + Rz_gate_i.theta), 2 * np.pi
                            )
        ...
        for line in self.qreg:
            ...
            for gate in line:
                ...
                Adaptive = Adaptive_decomposition(
                    gate.matrix,
                    self.energy_level_graph,
                    (algorithmic_cost, total_cost),
                    self.dimension,
                )
