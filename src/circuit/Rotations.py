...
class R:
    ...
    def __init__(self, theta, phi, o_lev_a, o_lev_b, dimension):
        ...
        Identity[self.lev_a, self.lev_a] = (
            np.cos(theta / 2) * Identity[self.lev_a, self.lev_a]
        )
        Identity[self.lev_b, self.lev_b] = (
            np.cos(theta / 2) * Identity[self.lev_b, self.lev_b]
        )
    ...
    def cost(self):
        ...
        E = (
            ...
            + 1 * abs(np.mod(abs(theta_on_units) + 0.25, 0.5) - 0.25)
        ) * 1e-04
        return E
...
class Rz:
    ...
    def __init__(self, theta, o_lev, dimension):
        ...
        self.theta = self.theta_corrector(
            theta
        )  # TODO DISCUSS IF THIS SHOULD GO THROUGH THE CORRECTOR
        ...
        Identity[self.lev, self.lev] = (
            np.exp(-1j * theta) * Identity[self.lev, self.lev]
        )
