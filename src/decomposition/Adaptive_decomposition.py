...
class Adaptive_decomposition:
    ...
    def execute(self):
        ...
        try:
            ...
            (
                ...
                best_cost,
                final_graph,
            ) = self.TREE.retrieve_decomposition(self.TREE.root)

            if matrices_decomposed != []:
                matrices_decomposed = self.Z_extraction(
                    matrices_decomposed, final_graph
                )
            else:
    ...
    def Z_extraction(self, decomposition, placement):
        ...
        U_ = decomposition[
            -1
        ].U_of_level  # take U of last elaboration which should be the diagonal matrix found
        ###########################################################################################################
        ...
        # @=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=#@=
            ...
            for i in range(
                dimension
            ):  # TODO take care of this variable because imported globally
    ...
    def DFS(self, current_root, level=0):
        ...
        if (
            not not_diag
        ) and valid_diag:  # if is diagonal enough then somehow signal end of algorithm
            # print("condition 2")
        ...
        for c in range(dimension):
            ...
            for r in range(c, dimension):
                ...
                for r2 in range(r + 1, dimension):
                    ...
                    if abs(U_[r2, c]) > 1.0e-8 and (
                        abs(U_[r, c]) > 1.0e-18 or abs(U_[r, c]) == 0
                    ):
                        ...
                        decomp_next_step_cost = (
                            cost_of_pi_pulses
                            + gate_cost
                            + current_root.current_decomp_cost
                        )
                        ...
                        branch_condition_2 = (
                            current_root.max_cost[0] - next_step_cost
                        )  # FIRST IS ALGORITHMIC COST

                        if (
                            branch_condition > 0 or abs(branch_condition) < 1.0e-12
                            ...
                            if (
                                new_placement.nodes[r]["lpmap"]
                                > new_placement.nodes[r2]["lpmap"]
                            ):
                                phi = phi * -1
                            ...
                            physical_rotation = gate_chain_condition(
                                pi_pulses_routing, physical_rotation
                            )
