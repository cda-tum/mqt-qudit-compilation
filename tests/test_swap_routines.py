...
class Test(TestCase):
    ...
    def test_pi_swap_routine(self):
        ...
        i = 2
        # swapping 6 wih 0
        cost, pi_swapping_list, new_graph = pi_swap_routine(
            list_to_swap, i, next_i, self.graph_1, len(list(self.graph_1.nodes))
        )

    def test_route_states2rotate(self):
        gate = R(np.pi, 0, 0, 6, 9)
        cost_of_pi_pulses, pi_pulses_routing, placement = route_states2rotate(
            gate, self.graph_1
        )
